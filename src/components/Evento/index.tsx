...
import useDeletaEventos from "../../state/hooks/use_deleta_eventos";
...
const Evento: React.FC<{ evento: IEvento}> = ({ evento}) => {
  ...
  const excluirEvento = useDeletaEventos()
  ...
  return (<div className={estilos.join(' ')}>
    ...
    <i className="far fa-times-circle fa-2x" onClick={() => excluirEvento(evento)}/>
  </div>)
}
